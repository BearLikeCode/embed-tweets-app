...
function App() {
  ...
  useEffect(() => {
    ...
    console.log((tweets?.length === comparedTweets?.data?.length && tweets?.data?.map(tweet => tweet.text).every((value, index) => value === comparedTweets?.data?.map(tweet => tweet.text)[index])))
    if (comparedTweets?.data === undefined || !(tweets.length === comparedTweets?.data?.length && tweets?.data.map(tweet => tweet.text).every((value, index) => value === comparedTweets?.data?.map(tweet => tweet.text)[index]))) {
